 ...
 * 				完全二叉树(complete BT)：对于深度为K的，有n个结点的二叉树，当且仅当其每一个结点都与深度为K的满二叉树中编号从1至n的结点一一对应时称之为完全二叉树。
 ...
 * 仿照二叉树的非递归层序遍历算法，利用队列来实现层序地创建新节点。
 *
 *
 *
 *
 */
{
	...
	BinTreeNode<T>* pn = nullptr;
	int order = 1;
	LinkQueue<BinTreeNode<T>* > q = LinkQueue<BinTreeNode<T>* >();
	q.enQueue(root);
	while(order<=n)
	{
		q.delQueue(pn);
		pn = new BinTreeNode<T>(order,nullptr,nullptr);
		order++;
		q.enQueue(pn->left);
		q.enQueue(pn->right);
	}
