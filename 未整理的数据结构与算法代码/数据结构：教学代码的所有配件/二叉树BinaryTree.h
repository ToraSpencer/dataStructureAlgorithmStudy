...
#include "链式队列LinkQueue.h"				// 链队列
#include "二叉树节点BinTreeNode.h"			// 二叉树结点类
...
public:
	...
	void InsertLeftChild(BinTreeNode<ElemType> *p, const ElemType &e);			// 插入一个结点e作为p的左孩子
	void InsertRightChild(BinTreeNode<ElemType> *p, const ElemType &e);			// 插入一个结点e作为p的右孩子
	...
	//查找算法。
	BinTreeNode<ElemType> *Find(const ElemType &e) const;		// 查找元素e，查找成功返回结点指针，否则返回NULL
...
template <class ElemType>
void CreateBinaryTree(BinTreeNode<ElemType> *&r, ElemType pre[], ElemType in[], int preLeft, int preRight, int inLeft, int inRight);
