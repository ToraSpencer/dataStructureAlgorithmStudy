...
//	辅助函数:
	...
	BinTreeNode<ElemType> *Parent(BinTreeNode<ElemType> *r, const BinTreeNode<ElemType> *p) const;			// 在以r为根的二叉排序树中求p的双亲
...
public:
	...
	BinarySortTree(const ElemType &e);						// 建立以e为根的BST
	BinarySortTree(BinTreeNode<ElemType> *r);				// 建立以r为根的BST
	BinarySortTree(const BinarySortTree<ElemType> &copy);	// 拷贝构造函数
	virtual ~BinarySortTree();

	//get/set方法：
	BinTreeNode<ElemType> *GetRoot() const;				// 求树的根
	...
	BinarySortTree<ElemType> &operator=
		(const BinarySortTree<ElemType>& copy);
};
